...
class expPayment extends commando.Command
{
    constructor(client)
    {
        super(client, {
            ...
            group: 'payment',
            memberName: 'setexp',
            description: '(Admin only) Set the exp of an account: e!exp [recipient\'s ID] [amount] '
        });
    }
                ...
                con.then(function(conn){
                    ...
                    amount = parseInt(amount, 10);
                        ...
                        { 
                            ...
                            {
                                message.reply("You can't set exp to be negative, that would mean they're dead.");
                                return 0;
                            }
                        }
                    }).then(function(y){
                        ...
                        if (z == 0)
                        {
                            ...
                            chnl.send(message.author.username +" set "+name + " ID["+accId+"] to "+amount+" exp.");
                            connection.query("INSERT INTO log (name1, account1, action, name2, amount, date) VALUES ('"+name+"', '"+accId+"', 'Set experience' ,'" +message.author.username +"', '"+ amount+"', '"+new Date().toISOString().slice(0, 19).replace('T', ' ')+"')");
